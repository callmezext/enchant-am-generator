...
(async () => {
  console.log('--- Testing Generator V2 for admin@enchant.id ---');
  console.log('Using account:', acc.email, 'Points:', acc.adPoints);

  const sel = await req('POST', '/api/generator-v2/select-email', { emailAddress: 'admin@enchant.id' });
  console.log('Select email response:', JSON.stringify(sel, null, 2));

  for (let i = 0; i < 5; i++) {
    await new Promise(r => setTimeout(r, 2000));
    const poll = await req('GET', '/api/generator-v2/poll-email');
    console.log(`Poll #${i+1}:`, poll);
  }
})();
