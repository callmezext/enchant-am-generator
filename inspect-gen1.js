  ...
  return new Promise((resolve) => {
    const r = https.request(url, { method, headers, timeout: 30000, agent }, (res) => {
      ...
      res.on('end', () => {
        ...
        resolve({ status: res.statusCode, text: raw.toString('utf-8') });
      });
    });
...
(async () => {
  ...
  // Fetch /dashboard/generator (v1)
  console.log('Fetching /dashboard/generator...');
  const gen1 = await req('GET', '/dashboard/generator');
  console.log('Status /dashboard/generator:', gen1.status, 'len:', gen1.text.length);

  // Extract chunks from generator (v1)
  const chunks = [...gen1.text.matchAll(/\/_next\/static\/chunks\/[^"']+\.js/g)].map(m => m[0]);
  console.log('Chunks:', chunks);

  for (const chunk of chunks) {
    const res = await req('GET', chunk);
    if (res.text.includes('/api/')) {
      console.log(`\n=== Chunk ${chunk} ===`);
      const apis = [...res.text.matchAll(/["'`](\/api\/[^"'`\s]+)["'`]/g)].map(m => m[1]);
      console.log([...new Set(apis)]);
      
      // Search for email verification patterns
      if (res.text.includes('verify') || res.text.includes('link') || res.text.includes('custom') || res.text.includes('manual')) {
        console.log('Has verify/link/custom/manual keywords!');
      }
    }
  }
})();
